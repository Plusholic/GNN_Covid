...
class ProposedSTGNN(nn.Module):
  ...
  def __init__(self,
    ...
    # self.theta3 = nn.Parameter(torch.FloatTensor(spatial_out_channels + spatial_channels,
    #                                              out_channels))
    
    # self.conv2 = nn.Conv2d(in_channels=out_channels + spatial_channels,
    #                        out_channels=out_channels,
    #                        kernel_size=(1, temporal_kernel))
    
    self.conv2 = nn.Conv2d(in_channels=spatial_hidden_channels + spatial_channels,
                           out_channels=out_channels,
                           kernel_size=(1, temporal_kernel))
    if batch_norm:
  ...
  def forward(self, input, adj):
    ...
    # Graph Conv 1
    h1 = torch.matmul(h0, self.theta1)
    h1 = torch.einsum('ij,jklm->kilm', [adj, h1.permute(1, 0, 2, 3)]) # (n_nodes, batch_size, time_steps, in_feautures)
    h1 = F.relu(h1)
    h1 = F.dropout(h1, p=self.drop_rate, training=self.training)
    h1 = torch.cat([h1, h0], dim=-1)
    ...
    # Graph Conv 2
    h2 = torch.matmul(h1, self.theta2)
    h2 = torch.einsum('ij,jklm->kilm', [adj, h2.permute(1, 0, 2, 3)]) # (batch_size, n_nodes, time_steps, in_feautures)
    h2 = F.relu(h2)
    h2 = F.dropout(h2, p=self.drop_rate, training=self.training)
    h2 = torch.cat([h2, h0], dim=-1)
    ...
    # input(" ctrl + c ")
    # Graph Conv 3
    # print(self.theta3.shape)
    # print(h2.shape)
    # h3 = torch.matmul(h2, self.theta3)
    # h3 = torch.einsum('ij,jklm->kilm', [adj, h3.permute(1, 0, 2, 3)]) # (batch_size, n_nodes, time_steps, in_feautures)
    # h3 = F.relu(h3)
    # h3 = F.dropout(h3, p=self.drop_rate, training=self.training)
    # h3 = torch.cat([h3, h0], dim=-1)
    # h3 = self.conv2(h3.permute(0, 3, 1, 2)).permute(0, 2, 3, 1)
    # h3 = F.relu(h3)
    # output = self.fc(h3.reshape(h3.size(0), h3.size(1), -1))
    # print(output)
